...
static void init_ssp(void)
{
 ...
 // Inter Integrated Circuit "eye-squared-C"
// - Synchronous, multimaster/multi-slave (controller/target)
// serial commununication bus.
...
int main (void) {
	...
	//int32_t TEMPERATURE_STRING_MAX
	//int32_t temperature = 0;

    init_i2c(); // lampki // on jest gotowy wysylamy do niego komende // zakresy itd.
	init_ssp();
	init_adc();
	oled_init();
	//light_init();
	...
	/// works without ???
    /// init_i2c(); // ? communication interface
	/// init_ssp(); // communication interface
	/// Needed for communication with ..
	/// checks volts on inputs
    /// init_adc(); // ?
	...
	/// OLED does not need communication with any interface.
    oled_clearScreen(OLED_COLOR_WHITE);

    //oled_putString(
    //	SCREEN_SPACING, SCREEN_FONT_Y + SCREEN_SPACING,
	//	(uint8_t*)"Temp  : ",
	//	OLED_COLOR_WHITE, OLED_COLOR_BLACK
	//);

	/// Filling whole screen.
	/// oled_fillRect(0, 0, 95, 63, OLED_COLOR_BLACK);
    ...
    while (1) {
    	...
    	oled_fillRect(0, 0, 31, 31, OLED_COLOR_BLACK);

    	//oled_fillRect((1+9*6),1, 80, 8, OLED_COLOR_WHITE);
    	//oled_putString(
    	//	SCREEN_SPACING + SCREEN_FONT_X * 7, SCREEN_FONT_Y + SCREEN_SPACING,
    	//	buf,
		//	OLED_COLOR_WHITE, OLED_COLOR_BLACK
		//);
    }
